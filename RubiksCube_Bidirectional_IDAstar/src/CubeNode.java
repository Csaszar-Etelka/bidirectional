 ...
 * during BD_IDA* to determine which node has the lower heuristic.
 */
public class CubeNode implements Comparable<CubeNode> {
	...
	public static ArrayList<CubeNode> getSuccessors(CubeNode node) {
		...
		for (Map.Entry<Character, int[]> face : Cube.FACES.entrySet()) {
			...
			possibleHeuristics[0] = BD_IDAStar.corners[encodedCorner];
			possibleHeuristics[1] = BD_IDAStar.edgesSetOne[encodedEdgeSetOne];
			possibleHeuristics[2] = BD_IDAStar.edgesSetTwo[encodedEdgeSetTwo];
			...
			successors.add(new CubeNode(newState, BD_IDAStar.corners[encodedCorner], node.path + face.getKey() + "1")) ;
		}
		return successors;
	}
